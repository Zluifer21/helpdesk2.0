...
export class TicketsService {
  ...
  this.headers=new HttpHeaders({'Authorization':'Bearer '+localStorage.getItem('token')})  }
   ...
   {
     return this.http.get(API.url+'ticketsAll', {headers: this.headers});
   }
   ...
   createticket_new(ticket):Observable<any>
   {
    ...
    formData.append('phone', ticket.phone);
    formData.append('expiration', ticket.expiration);
    formData.append('area_id', ticket.area_id);
   ...
   for (let i=0; i<ticket.subjects_ids.length;i++)
   {
    formData.append('subjects_ids[]',ticket.subjects_ids[i]);
   }
   ...
   {
     let headers= new HttpHeaders({'Authorization':'Bearer '+localStorage.getItem('token')});
     return  this.http.put(API.url+'ticket/'+ticket.id,ticket,{headers});
   }
    UpdateTicket_reasinged(ticket)
 {
   //console.log(ticket);
   let headers= new HttpHeaders({'Authorization':'Bearer '+localStorage.getItem('token')});
   return  this.http.post(API.url+'ticketReasinged',ticket,{headers});
 }

 ticketByStatus(status_id):Observable<any>
 {
   return this.http.get(API.url+'ticketByStatus/'+status_id, {headers: this.headers});
 }

 myTicketsAll():Observable<any>
 {
   return this.http.post(API.url+'myTicketsAll', {headers: this.headers});
 }
}
